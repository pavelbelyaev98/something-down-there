...
- **Environment:** Triplanar soil/turf shader (`GardenGround`) with purchased Pure Nature plain grass, nearly overhead noon sun and matching sun disc, cyan sky gradient. `SurfaceGrassRenderer` batches BK's low-poly cards, checks their wider footprints against remaining soil and regenerates from terrain after load; the old Unity grass assets are retired. BK's manager supplies wind with lighting overrides disabled. `SunPresentationSetup` retains the global URP saturation profile and true black underground; depth/opaque textures and depth priming support the purchased shaders.
