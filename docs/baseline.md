...
- **Environment:** Existing triplanar soil/turf and excavation-aware BK grass sit inside layered reservoir cliffs, a pale former waterline, forested upper banks, a closed dry rockfall gorge and distant mountains. Low rim stones mark diggable ground and keep the service bay clear. `ReservoirEnvironmentSetup` authors ordinary prefab instances with LODs, shared material overrides and near collision; scenery owns no gameplay/save state. `SunPresentationSetup` owns warm high sunlight, a blue clouded sky and restrained saturation. Scenery haze is local, preserving true black underground; BK's manager supplies wind only. Depth/opaque textures and depth priming support the purchased shaders.
