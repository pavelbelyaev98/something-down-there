...
## Playtest iteration: taut cable and forceful recovery

- **Objective / concept 05 §3, 09 §§3/7:** visibly load the cable, wind up against a jam, tear a local chunk free, and let the ensuing momentum smash subsequent contacted dirt. Preserve whole-object ownership, automatic recovery, physical rotation, tunnel contacts, gentle pad arrival and comfort-safe debris.
- **Diagnosis:** `WinchRopeView.Draw` adds the guide-to-load spring extension back into the paid cable, hiding strain. `SalvageWinch.Contacts` forgets every impact's incoming speed and restarts its contact timer after useful movement. Strong damping and the cruise speed cap consume the release burst.
- **Changes:** retain the existing route, dynamic body, spring and terrain commit path. Derive cable take-up from the reel alone and visual tautness from actual extension; loaded spans straighten between collision supports. Increase cruise/release response with force bounded by speed headroom and gentler release damping. Sweep CCD and stored incoming velocity keep the impact available after collision response, including when the load overtakes its guide. Authorize immediate rupture only for sufficiently energetic normal impacts, scale local removal with effort, and return only the energy left after breaking soil. Reuse pooled break feedback; never clear a predicted hull or teleport the load.
- **Edge cases:** reject speculative/separated and glancing contacts, never cut permanent obstacles or distant dirt, process at most one committed edit per step, spend impact energy once, reset transient contact samples on pause/restore, preserve saved motion (including bounded collision-driven spins through `ExtractionSnapshot` validation), and damp delivery separately. Rope simulation remains bounded and keeps live collision/momentum through bends.
- **Acceptance:** hauling and jammed cable spans are taut at the hook; slow/brief contact still cannot dig; a real break produces a faster physical surge; a fast follow-up collision breaks sooner than the jam delay and retains only residual momentum; bends, blocking commons/lamps, permanent walls, pause/reload and pad/display recovery remain correct. Run core rope/extraction and recovery checks, inspect disposable MainGame hauling, compile and validate, and build a fresh Windows player.
- **Verification:** the core EditMode suite, final extraction/save and rope checks, all recovery scenarios and focused find release/settling regressions pass. Disposable MainGame review shows wind-up, local rupture, faster release and rapid follow-up breaks with pooled debris. Scripts compile cleanly and build validation passes; the fresh Windows player builds with no errors and only the existing optional disabled Pipeline runtime warning.

## Manual unique uncovering and visible recovery marks

- **Objective / concept 05 §3, 08 §5:** manually excavate surrounding soil before marking a unique. Show the eligible surface point while hovering, hold progress there, and a distinct confirmed mark following the item throughout recovery.
- **Diagnosis / changes:** `BuriedFind.TryGetCoveringSoil` currently redirects strokes aimed at uniques as well as commons. Exclude unique identities there while retaining ordinary soil cuts, common assistance, exposure thresholds and the accepted post-mark rupture behavior. Share `FindExtractionInteraction` eligibility between the hold and preview. Extend `WinchRopeView` with one reusable, depth-tested surface glyph driven by hover or the existing job attachment; `SalvageWinch` owns its lifecycle. Restore its orientation from the actual payload collider at the saved attachment, avoiding extra save ownership or fields.
- **Edge cases:** hidden/ineligible/out-of-reach finds, equipment placement and menus cannot preview or mark; cancelling a hold never leaves a confirmed mark. Active jobs display their one attachment, follow rotation and survive reload. Completion, failed planning and clearing/restoring the world remove stale markers. Glyphs have no collider and cannot block digging or rope contacts.
- **Acceptance:** directly aimed unique strokes leave soil and fuel untouched while deliberate surrounding strokes increase exposure; common uncover assistance still works, including with a full bag. Verify preview eligibility, cancellation, confirmed attachment and checkpoint continuity alongside physical recovery. Inspect the actual glyph in MainGame, compile and validate, then rebuild Windows.
- **Rendering:** use the forward transparent pass with ordinary depth testing for the cutout glyph. The scene's depth priming rejects an opaque glyph without its own depth prepass; the marker must remain visible on the find while still being occluded by real geometry.
- **Verification:** discovery regressions and all recovery scenarios pass, including manual unique excavation, normal common assistance, hover occlusion/menu cancellation, rotated checkpoint marks and marker cleanup on arrival. MainGame review confirms the segmented hover ring, filling hold and checked confirmation. Scripts and marker shader compile cleanly; validation and the fresh Windows build pass with only the existing optional Pipeline runtime warning.
