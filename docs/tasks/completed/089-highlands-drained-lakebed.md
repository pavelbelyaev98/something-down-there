...
**Status:** complete. MainGame's surroundings are regenerated by `LakebedSiteSetup` from the Highlands demo's east river arm with the demo's sky and baked reflections: a widened lake and one drained lakebed section the player cannot leave (walls + flight ceiling), holding the round meadow opening behind a pebble border and grass fade, with the tested station cluster on the south rim; scenery unseen from the play area is removed.
