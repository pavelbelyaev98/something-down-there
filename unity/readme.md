...
- The dig plot's cap, damp band and topsoil options live in `LakebedSiteSetup.DigGround.cs` (the muted original soil colour, `Soil_Albedo_Muted.png`, is derived once by `GroundTextureSetup.MutedSoilAlbedo`; delete it to regenerate), the dig boundary options in `LakebedSiteSetup.Boundary.cs`; rerun full site setup, then rebake occlusion, after changing them. The demo's own terrain layers stay untouched: lakebed-only layers are appended after them.
