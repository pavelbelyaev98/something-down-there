...
- **Test Once, at the End:** While implementing and iterating, rely on compilation, the Editor console and targeted Play Mode/visual checks; do not run test suites after every change. Run the relevant suite (`tools/test-changed.ps1`, `-Full` for broad changes) once when the work is finished, rerun only the failing classes after fixes, then build. Never run tests and a build at the same time.
- **Completion Protocol:** A task is complete only when:
  1. Code compiles warning-free and passes relevant tests (including any new high-value tests), run once after the work is finished.
...
- **Decentralized Asset Tracking:** Do **not** maintain a centralized asset ledger. Document art/audio assets minimally in their local folder: `art/<name>/README.md` (5–8 line card stating: Item, Purpose, Source/License, Unity Path, Status). Developer utility setup/license notes belong in `unity/readme.md`, not `art/`.
  - This includes art made by editor code. Original generated geometry or textures (props, meshes, procedural textures) get a card in the same change that creates them. Tinted material, terrain-layer or prefab copies of approved pack content are listed on that pack's card. When such an asset is deleted, remove its card or entry in the same change.
...
- **Unsaved Editor scenes:** The user authorizes discarding or replacing unsaved scene/playtest edits without another confirmation. Save intentional task changes explicitly first; then use `tools/prepare-editor-tests.cs` through CLI `eval_file` before manual test runs (`test-changed.ps1` does this automatically). It opens a clean empty scene and closes the previous scenes without saving, so the test runner cannot block on the save dialog. This does not authorize overwriting the protected recovery scene.
